...
mod tests {
    ...
    fn origin_from_header_accepts_any_scheme() {
        let list = vec!["chrome-extension://abc".to_string()];
        let o = origin_from_header(Some("chrome-extension://abc/"));
        assert_eq!(o.as_deref(), Some("chrome-extension://abc"));
        assert!(is_origin_allowed(o.as_deref().unwrap(), &list));

        assert!(is_well_formed_origin("https://app.example"));
        assert!(is_well_formed_origin("moz-extension://id"));
